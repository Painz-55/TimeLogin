...
document.getElementById("addBoss").onclick = () => {
  config.bosses.push({ nome: "New Boss", tempo: 60 });
  saveConfig();
};
...
function saveGlobal() { set(ref(db, "config/timers"), config.timers); }
function saveConfig() { set(ref(db, "config/bosses"), config.bosses); }
...
function triggerTimerFinished(i) {
  clearInterval(intervals[i]);
  let timerDiv = document.querySelectorAll(".timer")[i];
  if (timerDiv) timerDiv.classList.add("finished");
  playAlarm();
}
...
exitObs.onclick = () => {
  document.querySelector(".rightPanel").style.display = "";
